...
| After a port-bind failure, the app comes up but throws `UnknownHostException: postgres` (or another service name) — **looks like a DB outage** | The failed bind left a **half-created container with a broken network attachment**: the app is running but not properly joined to the compose network, so it can't resolve sibling service names. `dc start` reuses that broken container | Recreate it, don't just start it: `dc up -d --force-recreate <service>` (or `dc down && dc up -d`). `dc start`/`dc restart` alone will **not** fix it — the container must be recreated to re-attach the network. |
...
| App or DB unreachable; an unrelated container holds `:8080` or `:5432` | Port squatting by another project | `docker ps` to find the squatter; stop it, or re-port Atlas via `ATLAS_CORE_PORT` / (dev) `POSTGRES_PORT` in `.env`. |
